...
# I hated that Ryan had us using these double back slasshes when we started Ryan
# I no longer read csv files this way

celldeath = read.csv(file="U:\\ECON 6320 Fall 2016\\Cellphone_2012_homework.csv", head=TRUE)

# A slight modification to the code from 6 years ago
library(haven)
celldeath <- read_dta('cellphone.dta')
